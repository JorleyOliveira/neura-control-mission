...
2. CENTER (~420px): the JARVIS conversation.
   ...
   - VOICE ORB pinned at the bottom center: a glowing circle with a mic icon.
     ...
     Agora channel using agora-rtc-sdk-ng (`AgoraRTC.createClient({mode:"rtc"})`): when
     `token_enabled` is true, first GET `{API}/api/voice/rtc-token?channel={channel}&uid=0`
     and call `join(app_id, channel, token, 0)`; otherwise `join(app_id, channel, null)`.
     Then create and publish a local microphone track. The orb glows "listening" while joined;
     click again to leave (stop and close the mic track). Spoken words are handled entirely by
     the backend voice agent — the orb only joins/leaves the channel.
